...
export abstract class DataflowSolver<D> {
  ...
  protected init(): void {
    ...
    const edgePoint = new PathEdgePoint<D>(
      this.problem.getEntryPoint(),
      this.zeroFact,
    );
  ...
  protected getAllCalleeMethods(callNode: ArkInvokeStmt): Set<ArkMethod> {
    const caller =
      callNode.getCfg()?.getDeclaringMethod() ?? this.problem.getEntryMethod();
    const callerNode = this.CHA.getCallGraph().getCallGraphNodeByMethod(
      caller.getSignature(),
    );
    if (!callerNode) {
      return new Set();
    ...
    for (const callSite of callSites) {
      const signature = this.CHA.getCallGraph().getMethodByFuncID(
        callSite.calleeFuncID,
      );
  ...
  private updateQueuePeaks(): void {
    if (!this.statistics) return;
    const immediate =
      this.immediateWorkList.length - this.immediateWorkListHead;
  ...
  protected applySummaryToIncomingCallers(
    ...
    for (const callerEdge of callerEdges) {
      if (
        callerEdge.edgeEnd.node !== callEdgePoint.node ||
        !this.problem.factEqual(callerEdge.edgeEnd.fact, callEdgePoint.fact)
      )
        continue;
  ...
  protected processCallNode(edge: PathEdge<D>): void {
    ...
    const declaringFile = invokeStmt
      .getInvokeExpr()
      .getMethodSignature()
    ...
    for (const callee of callees) {
      ...
      const firstStmt = callee.getCfg()!.getStartingBlock()!.getStmts()[
        callee.getParameters().length
      ];
    ...
    for (const fact of set) {
      this.propagate(
        new PathEdge<D>(start, new PathEdgePoint<D>(returnSite, fact)),
      );
    }
  ...
  protected callNodeFactPropagate(
    ...
    for (const exitEdgePoint of this.summaryStore.getEndSummaries(
      startEdgePoint,
    )) {
      const returnFlowFunc = this.problem.getExitToReturnFlowFunction(
        exitEdgePoint.node,
      ...
      for (const returnFact of returnFlowFunc.getDataFacts(
        exitEdgePoint.fact,
      )) {
  ...
  protected hasPendingEdge(): boolean {
    return (
      this.immediateWorkListHead < this.immediateWorkList.length ||
