...
class SemanticPointMap<D, V> {
    ...
    get(point: PathEdgePoint<D>): V | undefined {
        const bucket = this.index.get(point.node)?.get(
            this.semantics.factHash(point.fact)
        );
    ...
    private pointsEqual(left: PathEdgePoint<D>, right: PathEdgePoint<D>): boolean {
...
export class SummaryStore<D> {
    ...
    getIncoming(calleeEntry: PathEdgePoint<D>): ReadonlySet<PathEdge<D>> | undefined {
    ...
    getCallSummaries(
        ...
        return this.callSummaries.get(callPoint)?.byReturnSite
            .get(returnSite)?.points ?? [];
    }
