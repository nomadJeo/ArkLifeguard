...
//自定义模块导入
import { UICallbackInfo, UIEventType, ComponentInfo } from "./LifecycleTypes";
...
export class ViewTreeCallbackExtractor {
  ...
  public extractFromComponent(componentClass: ArkClass): UICallbackInfo[] {
    ...
    if (!viewTree) {
      console.log(
        `[ViewTreeCallbackExtractor] No ViewTree for ${componentClass.getName()}`,
      );
      return callbacks;
    }
    ...
    if (!root) {
      console.log(
        `[ViewTreeCallbackExtractor] Empty ViewTree for ${componentClass.getName()}`,
      );
      return callbacks;
    }
  ...
  private extractNodeCallbacks(
    ...
    for (const [attributeName, attributeValue] of node.attributes) {
      ...
      // 尝试解析回调方法
      const callbackMethod = this.resolveCallbackMethod(
        attributeValue,
        componentClass,
      );
      if (!callbackMethod) {
      ...
      const callbackInfo: UICallbackInfo = {
        ...
        relatedStateValues: this.collectRelatedStateValues(
          node,
          attributeValue,
        ),
        viewTreeNode: node,
  ...
  private resolveCallbackMethod(
    ...
    for (const value of relatedValues) {
      ...
      if (method) {
        console.log(
          `[ViewTreeCallbackExtractor] Resolved callback: ${method.getName()}`,
        );
        return method;
      }
    }
    ...
    if (lambdaMethod) {
      console.log(
        `[ViewTreeCallbackExtractor] Resolved lambda callback: ${lambdaMethod.getName()}`,
      );
      return lambdaMethod;
    }

    console.log(
      `[ViewTreeCallbackExtractor] Failed to resolve callback from ${relatedValues.length} values`,
    );
    return null;
  }
  ...
  private resolveFromFieldRef(
    ...
    // 这种情况较复杂，暂时不处理
    console.log(
      `[ViewTreeCallbackExtractor] Field ref '${fieldName}' is not a method`,
    );
    return null;
  }
 ...
 * @returns UI 回调信息数组
 */
export function extractUICallbacks(
  scene: Scene,
  componentClass: ArkClass,
): UICallbackInfo[] {
  const extractor = new ViewTreeCallbackExtractor(scene);
