...
| ExceptionFlow | 14 | 显式/隐式抛出、catch 参数、重抛、跨过程异常、finally 完成顺序及 Promise 拒绝 |
...
异常流样例将同步语言级异常与 Promise 拒绝分开建模：前者覆盖 `throw`、`catch`、`finally` 和跨过程异常返回，后者覆盖 `await`、`.catch()` 与 `.finally()`。`UncaughtThrowStopsFlow` 和 `NoThrowSkipsCatch` 是控制流安全负例，用于约束不可达路径误报。`CaughtNullDereference` 仍按“发生了语义解引用”标为正例；异常被 catch 只改变运行时后果，后续报告层可以再区分已捕获的功能失败风险与未捕获崩溃。
...
在 ArkAnalyzer 项目根目录运行全部 66 个样例：
